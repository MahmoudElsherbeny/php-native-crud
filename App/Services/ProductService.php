...
abstract class ProductService
{
    ...
    public function validate() 
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $type = $validator->check_input('type', $_POST["type"], ['required', 'in_array' => ['dvd', 'book', 'furniture']]);
            if($_POST["type"] == 'dvd') {
        ...
        if(!empty($errors)) {
            session_start();
            $_SESSION['sku_error'] = $errors['sku'];
            $_SESSION['name_error'] = $errors['name'];
            $_SESSION['price_error'] = $errors['price'];
            $_SESSION['type_error'] = $errors['type'];
        }
